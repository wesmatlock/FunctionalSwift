public struct Changeable<A> {
  ...
  public func write<Value: Equatable>(_ newValue: Value, at keyPath: WritableKeyPath<A, Value>) -> Changeable<A> {
    guard newValue != value[keyPath: keyPath] else {
      return Changeable(value: value, hasChanges: false)
    }
...
public func write<Value: Equatable, Root>(_ value: Value,
  ...
  return { root in
    guard value != root[keyPath: keyPath] else {
      return Changeable(value: root, hasChanges: false)
    }
