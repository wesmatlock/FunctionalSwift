...
final class IOTests: XCTestCase {
  ...
  func testFlatMap() {
    let muliplyWithHundered: (Int) -> IO<Int> = { number in IO { number * 100 } }

    let result = IO<Int> { 10 }
      .flatMap(muliplyWithHundered)
      .flatMap(muliplyWithHundered)
      .flatMap(muliplyWithHundered)
      .unsafeRun()
  ...
  func testPure() {
    ...
    let result2 = IO<Double>.pure(106.7).unsafeRun()
    XCTAssertEqual(106.7, result2)
  }
